...
@Component({
  selector: 'app-general_membership_form',
  templateUrl: './general_membership_form.component.html',
  styleUrls: ['./general_membership_form.component.css']
})
export class General_membership_formComponent implements OnInit {
  date!: Date;
  // member!: MemberType[];
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      member_type: [{ value: 'Genaral Membership', disabled: this.isReadonly }],
    ...
    //   depend_phone: [''],
    //   relation: [''],
    // }, { validators: this.validatorsService.conditionalRequiredValidator('depend_name', ['depend_phone','relation']) });
  ...
  next() {
    switch (this.activeIndex) {
      ...
      case 4:
        this.fee_info = {
          admissionFee: this.f['admissionFee']
            ? this.f['admissionFee'].value
            : null,
          donationFee: this.f['donationFee']
            ? this.f['donationFee'].value
            : null,
          subscriptionFee: this.f['subscriptionFee']
            ? this.f['subscriptionFee'].value
            : null,
          subscriptionType: this.f['subscriptionType']
            ? this.f['subscriptionType'].value
            : null,
        };
        break;
    }
  ...
  successNotification() {
    this.dataServe
      .global_service(1, '/save_genral_form', this.personal_info)
      .subscribe((data: any) => {
        this.savedata = data;
        ...
        if (this.savedata.suc > 0) {
          ...
          Swal.fire(
            ...
            if (result.isConfirmed) {
                    this.router.navigate(['/home/print_general_form',encodeURIComponent(btoa(this.formNo))])
                  }
                });
