...
@Component({
  selector: 'app-super_topup_policy',
  templateUrl: './super_topup_policy.component.html',
  styleUrls: ['./super_topup_policy.component.css'],
  providers: [MessageService,DatePipe],
})
export class Super_topup_policyComponent implements OnInit {
  memb_opr: string = 'J';
  value!: string;
  date!: Date;
      ...
      data => {
        ...
        if (this.groupSaveData.suc > 0) {
          ...
          Swal.fire(
            ...
            if (result.isConfirmed) {
                    this.router.navigate(['/main/print_stp_form',encodeURIComponent(btoa(this.formNo))])
                  }
                });
        } else {
          Swal.fire(
