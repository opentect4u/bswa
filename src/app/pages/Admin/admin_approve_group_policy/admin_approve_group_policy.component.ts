...
@Component({
  selector: 'app-admin_approve_group_policy',
  templateUrl: './admin_approve_group_policy.component.html',
  styleUrls: ['./admin_approve_group_policy.component.css'],
  providers: [MessageService,DatePipe]
})

export class Admin_approve_group_policyComponent implements OnInit {
  userData: any = [];
  form!: FormGroup;
  tr_status:any = 'Y'
  tbFilterData: any = []
  ...
  ngOnInit() {
   ...
   get m() {
    return this.form.controls;
       ...
       submit(){
    this.dataServe.global_service(0,'/frm_list_gp_policy',`form_no=${this.m['form_no'].value}`).subscribe(data => {
  ...
  submit_search_gp(){
    ...
    this.dataServe.global_service(1,'/search_form_gp',dt).subscribe(data => {
    ...
    this.router.navigate(['/admin/gp_policy_view_form',encodeURIComponent(btoa(formNo)),encodeURIComponent(btoa(memb_id)),encodeURIComponent(btoa(phone_no))])
  }
