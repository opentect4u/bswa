...
@Component({
  selector: 'app-admin_approve_child_policy',
  templateUrl: './admin_approve_child_policy.component.html',
  styleUrls: ['./admin_approve_child_policy.component.css'],
  providers: [MessageService,DatePipe]
})
export class Admin_approve_child_policyComponent implements OnInit {
  userData: any = [];
  form!: FormGroup;
  tr_status:any = 'Y'
  tbFilterData: any = []
  ...
  get m() {
    return this.form.controls;
     ...
     submit(){
    this.dataServe.global_service(0,'/frm_list_child_policy',`form_no=${this.m['form_no'].value}`).subscribe(data => {
    ...
    this.pendingCount = this.userData.msg.filter((item: { approval_status?: string }) => item?.approval_status === 'P').length;
    this.rejectCount = this.userData.msg.filter((item: { approval_status?: string }) => item?.approval_status === 'R').length;
    this.approvedCount = this.userData.msg.filter((item: { approval_status?: string }) => item?.approval_status === 'A').length;
  ...
  submit_search_child(){
    ...
    this.dataServe.global_service(1,'/search_form_child',dt).subscribe(data => {
    ...
    this.router.navigate(['/admin/child_policy_view_form',encodeURIComponent(btoa(formNo)),encodeURIComponent(btoa(memb_id)),encodeURIComponent(btoa(phone_no))])
  }
  ...
  if (this.userData.msg && this.userData.msg.length > 0) {
    ...
    this.tbFilterData = this.userData.msg.filter((dt: any) => {
      if (flag === 'Y') {
        return dt.approval_status === 'P'; // Pending
      } else if (flag === 'R') {
        return dt.approval_status === 'R'; // Rejected
      } else if (flag === 'A') {
        return dt.approval_status === 'A'; // Approved
      } else {
        return true;
      }
    });
